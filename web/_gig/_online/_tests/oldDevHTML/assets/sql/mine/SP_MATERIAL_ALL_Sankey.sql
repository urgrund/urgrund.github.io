SELECT 
	...
	,case when RDP.[Source_Category] = 'Stockpile' then 'Middle' else 'Left' end as 'Start position'
	,case when RDD.[Destination_Description] in ('U/G Crusher__O', 'M35 Surface Ore Stockpile', 'M35 Surface Waste Stockpile', 'M35 Surface Stope Stockpile') then 'Right' else case when RDP.[Source_Category] = 'Stockpile' then 'Middle2' else 'Middle' end end as 'Finish position'
	...
	GROUP BY RDP.[Source_Category], RDP.[Source_MineArea_Code], RDP.[Source_Description_Code],RDP.[Source_Description], RDD.[Destination_Description_Code],RDD.[Destination_Description], RDD.[Destination_MineArea_Code] 