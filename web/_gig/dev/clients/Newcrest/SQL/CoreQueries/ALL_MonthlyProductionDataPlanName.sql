...
WHERE substring(ats.[shkey],1,4) > 2018 /*@Year and substring(ats.[shkey],5,2) = @Month*/ and MTS.MeasCode ='TONNE' and RDF.[Equipment_FunctionCode] = 'LOADING' and RDP.[Source_Category] like '%Production%'
--and RDP.[Source_MineArea_Code] like '%finn%' and left(ATS.[Location],4) = '4539' -- 
...
ORDER BY RDP.[Source_MineArea_Code] --left(ats. [SHkey],8), RDP.[Source_MineArea_Code] desc , ats.Location desc --,  RDP.[Source_MineArea_Code] desc 