...
SELECT [Date], [Shift], [Asset ID],  [Metric], [Secondy Activity Type], 
...
,MTS.MeasureValue as 'Values'
 ,RDP.[Source_Category] AS 'Secondy Activity Type'
		...
		WHERE ats.[shkey] between @StartD_S and @EndD_S  and MTS.MeasCode = 'TONNE' and RDF.[Equipment_FunctionCode] = 'HAULING' 
 ) as SC  
  PIVOT  
  (sum(SC.[Values])  
