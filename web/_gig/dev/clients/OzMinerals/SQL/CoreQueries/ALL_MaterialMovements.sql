SELECT 
	...
	,RDP.[Source_DevPrd] as 'Category'
    ,RDP.[Source_Description_Code] as 'Source Area'
	,replace(replace(RDP.[Source_Description],'_',''),'-',' ') as 'Source Labels'
	...
	left Join [dbo].[RD_DESTINATIONS]		as RDD on RDD.[DestinationCode] = ATS.[Destn]
	...
	GROUP BY RDP.[Source_DevPrd], ats.shkey, RDP.[Source_MineArea_Code], RDP.[Source_Description_Code],RDP.[Source_Description], RDD.[Destination_Description_Code],RDD.[Destination_Description], RDD.[Destination_MineArea_Code] 